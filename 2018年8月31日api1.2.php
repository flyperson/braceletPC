				...
				if(!empty($send_json["data"]["userphone"])){
					...
					if($result->num_rows > 0){
						...
						if($result2->num_rows > 0){
							...
							//获取用户的今天的分数及分组内的排名
							$nowdate = date("Y-m-d");
							$sql = "SELECT `user_id`,`score` FROM `usersetcount` WHERE `record_date`='".$nowdate."' ORDER BY `score` DESC";
							$result2 = $conn->query($sql);
							if($result2->num_rows>0){
								$stepcountdata = "";
								$i = 1;
								while($row2 = $result->fetch_row()){
									if($row2[0] == $tmpselfdata["userid"]){
										$stepcountdata["groupranking"] = $i;
										$stepcountdata["score"] = empty($row2[1])?0:$row2[1];
									}
									$i++;
								}
							}else{
								$stepcountdata["groupranking"] = 0;
								$stepcountdata["score"] = 0;
							}
							
							$restatus = 1;
							$remessage .= "登录成功";
							$redata = '{"userid":"'.$tmpselfdata["userid"].'","userphone":"'.$tmpselfdata["userphone"].'","username":"'.$tmpselfdata["username"].'","usericon":"'.$tmpselfdata["usericon"].'","regisrtationdate":"'.$tmpselfdata["regisrtationdate"].'","usergroup":"'.$tmpselfdata["usergroup"].'","userage":"'.$tmpselfdata["userage"].'","userscore":"'.$stepcountdata["score"].'","ranking_group":"'.$stepcountdata["groupranking"].'"}';
						}else{
							$remessage .= "密码号错误";
						}
					}else{
...
//			{"cmark":"GetSelfRunnumber","data":{"userid":"1"}}
			...
			case "GetGroupMember":
				...
				$sql = "SELECT `username`,`usericon` FROM `user` WHERE `usergroup`='".$send_json["data"]["usergroup"]."' ORDER BY `username`";
				$result = $conn->query($sql);
				if($result->num_rows>0){
					$tmpdata = "";
					while($row = $result->fetch_row()){
						$row[1] = empty($row[1])?"":$usericonpath.$row[1];
						$tmpdata .= ','.'{"username":"'.$row[0].'","usericon":"'.$row[1].'"}';
					}
					$tmpdata = substr($tmpdata, 1);
